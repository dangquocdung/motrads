...
class Sub_Categories extends Main
{
	...
	function exists($shop_id=0, $sub_category_id = 0)
	{
		$name = $_REQUEST['name'];
	...
	function is_valid_name( $name, $sub_category_id = 0)
	{
		...
		if ( strtolower( $this->sub_category->get_info( $sub_category_id )->name ) == strtolower( $name )) {
		// if sub category name is same with existing name,
			
			return true;
		} else if ( $this->sub_category->exists( array( 'name' => $name, 'shop_id' => $shop_id ))) {
		// if the sub category name is already existed in the system,
