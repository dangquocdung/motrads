...
class Items extends Main
{
	...
	function index()
	{
		...
		$data['items'] = $this->item->get_all_items($this->get_current_shop()->id, $pag['per_page'], $this->uri->segment(3));
		$data['pag'] = $pag;
	...
	function is_valid_name( $name, $item_id = 0 )
	{
		...
		if ( trim( strtolower( $this->item->get_info( $item_id )->name )) == strtolower( $name )) {
		// if item name is same with existing name,
			
			return true;
		} else if( $this->item->exists( array( 'name'=> $name, 'sub_cat_id' => $sub_cat_id ))) {
		// if the item name is already existed in the system,
