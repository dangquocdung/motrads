...
class Shops extends REST_Controller
{
	function get_get()
	{
		...
		if ($id) {
			...
			if($this->image->get_cover_image($id,'shop')->path == "") {
				$shop->cover_image_file = "image_not_found_big.png";
				$shop->cover_image_width = "400";
				$shop->cover_image_height = "300";
			} else {
			...
			$shop->price_decimal_place = $this->about->get_info(1)->price_decimal_place;
			...
			foreach ($shops as $shop) {
				...
				if($this->image->get_cover_image($shop->id,'shop')->path == "") {
					$shop->cover_image_file = "image_not_found_big.png";
					$shop->cover_image_width = "400";
					$shop->cover_image_height = "300";
				} else {
	...
	function get_categories($shop_id)
	{
		...
		foreach ($cats as $cat) {
			...
			if($this->image->get_cover_image($cat->id,'category')->path == "") {
				$cat->cover_image_file = "image_not_found_small.png";
				$cat->cover_image_width = "200";
	...
	function get_sub_categories($shop_id, $cat_id)
	{
		...
		foreach ($sub_cats as $sub_cat) {
			...
			//print_r($this->image->get_cover_image($sub_cat->id,'sub_category')->path); die;
			if($this->image->get_cover_image($sub_cat->id,'sub_category')->path == "") {
				$sub_cat->cover_image_file   = "image_not_found_small.png";
				$sub_cat->cover_image_width  = "200";
	...
	function get_items($shop_id, $sub_cat_id)
	{
		...
		foreach ($items as $item) {
			...
			//$items[$i]->images = $this->get_all_item_images($item->id);
	...
	function get_all_item_images($item_id) 
	{
		$imgs = $this->image->get_all_by_item($item_id)->result();
		...
		foreach ($imgs as $ig) {
			...
			if($ig->path == "") {
				$ig->path = "image_not_found_big.png";
				$ig->width = "400";
				$ig->height = "300";
			} else {
				$ig->path = $ig->path;
	...
	function feeds_get()
	{
		...
		$feeds = array();
		if ($count && $from) {
			$feeds = $this->feed->get_all_published($shop_id, $count, $from)->result();
		} else if ($count) {
			$feeds = $this->feed->get_all_published($shop_id, $count)->result();
		} else {
			$feeds = $this->feed->get_all_published($shop_id)->result();
		}
