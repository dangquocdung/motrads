	...
	function add_post()
	{
		...
		if(count($decode_data_array)>0) {
			...
			if($decode_data_array[0]->payment_method == "stripe") {
				...
				if($decode_data_array[0]->platform == "IOS") {
					...
					for($i=0;$i<count($decode_data_array);$i++) 
					{
						...
						$detail_data = array(
							...
							'item_attribute_id'     => $decode_data_array[$i]->basket_item_attribute_id,
					...
					for($i=0;$i<count($decode_data_array);$i++) 
					{
						$att_name = $this->attribute_detail->get_attribute_name($decode_data_array[$i]->basket_item_attribute_id);				
						$detail_data = array(
							'transaction_header_id' => $transaction_header_id,
							'shop_id'               => $decode_data_array[$i]->shop_id,
							'item_id'               => $decode_data_array[$i]->item_id,
				...
				$email            = $decode_data_array[0]->email;
				$phone            = $decode_data_array[0]->phone;
				$platform         = $decode_data_array[0]->platform;
				
				
				if($this->send_email_to_user($user_id,$payment_method,$shop_id,$transaction_header_id,$decode_data_array, $platform))
				{
					$send_user = true;
				}
				
				if($this->send_email_to_shop($user_id,$payment_method,$shop_id,$delivery_address,$billing_address,$email,$phone,$transaction_header_id,$decode_data_array, $platform))
				{
					$send_shop = true;
				}
...
//		
	...
	function send_email_to_user($user_id,$payment_method,$shop_id,$trans_id,$order_data, $platform)
	{
		if($payment_method == "cod" || $payment_method == "stripe"){
			...
			if($platform == "IOS") {
				...
				for($i=0;$i<count($order_data);$i++) 
					{
						if($order_data[$i]->basket_item_attribute != "") { 
						
							$order_items .= $i + 1 .". " . $order_data[$i]->name . "(" . $order_data[$i]->basket_item_attribute  .")" .
							" (Price : " .  price_format( $order_data[$i]->unit_price ) . $shop->currency_symbol . 
				...
				for($i=0;$i<count($order_data);$i++) 
				{
					...
					if($order_data[$i]->basket_item_attribute_id != "") {
						
						$att_name = $this->attribute_detail->get_attribute_name($order_data[$i]->basket_item_attribute_id);
						
						$order_items .= $i + 1 .". " . $order_data[$i]->name . "(". $att_name . ")" .
						" ( Price : " .  price_format( $order_data[$i]->unit_price ) . $shop->currency_symbol . 
						",  QTY : " . $order_data[$i]->qty . ") <br>";
			...
			$html = "<p>Hi ".$appuser->username.",</p>".
						...
						$order_items. "<br>".
						"Total Amount : " . price_format( $total_amount ) . " " . $shop->currency_symbol . "<br/><br/>".
						$trans_info.
			...
			if($platform == "IOS") {
				...
				for($i=0;$i<count($order_data);$i++) 
				{
					...
					if($order_data[$i]->basket_item_attribute != "") { 
						...
						$order_items .= $i + 1 .". " . $order_data[$i]->name ."(". $order_data[$i]->basket_item_attribute . ")" .
						" (Price : " .  price_format( $order_data[$i]->unit_price ) . html_entity_decode($shop->currency_symbol) . 
						", QTY : " . $order_data[$i]->qty . ") <br>";
			...
			$html = "<p>Hi ".$appuser->username.",</p>".
						"<p>Your order has been sent to the restaurant for the following dish at below : <br/><br/>".
						$order_items. "<br>".
						"Total Amount : " . price_format( $total_amount ) . " " . html_entity_decode($shop->currency_symbol) . "<br/><br/>".
	...
	function send_email_to_shop($user_id,$payment_method,$shop_id,$delivery_address,$billing_address,$email,$phone,$trans_id,$order_data, $platform)
	{
