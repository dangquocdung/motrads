			...
			?>
			  	<div class="form-group">
			  		...
			  		foreach($this->category->get_all($this->shop->get_current_shop()->id)->result() as $category){
			  			...
			  			echo ">".html_entity_decode($category->name)."</option>";	
			  		}
			  		?>
			  		</select>
			  	</div>
			  			...
			  			<?php
			  				foreach($this->sub_category->get_all_by_cat_id($cat_id)->result() as $sub_cat){
			  					...
			  					if($sub_cat_id == $sub_cat->id) 
			  						echo " selected ";
			  					echo ">".html_entity_decode($sub_cat->name)."</option>";
			  				}
			  			?>
			  		</select>
			  	</div>
			...
			<script type="text/javascript">
				...
				function drawGraphChart() {
					...
					var data = google.visualization.arrayToDataTable(<?php echo html_entity_decode($graph_items);?>);
					var options = {
						title: 'Total Touch Counts (All Items From ' + '<?php echo html_entity_decode($cat_name);?> and ' + '<?php echo html_entity_decode($sub_cat_name);?>)',
				...
				function drawPieChart() {
			     	...
			     	var data = google.visualization.arrayToDataTable(<?php echo html_entity_decode($pie_items);?>);
			     	var options = {
			       		title: 'Top 5 Popular Items From ' + '<?php echo html_entity_decode($cat_name);?> and ' + '<?php echo html_entity_decode($sub_cat_name);?>)',
			       		backgroundColor: { fill:'transparent' }
			     	};
